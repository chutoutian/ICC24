...
class ActorNetwork(object):
    ...
    of all actions.
    """
    def __init__(self, sess, state_dim, action_dim, learning_rate, neuron, layer, phy):
        self.sess = sess
    ...
    def create_actor_network(self):
        with tf.variable_scope('actor'):
            ...
            splits = []
            for i in range(self._phy):
                splits.append(tflearn.conv_1d(inputs[:, i:i+1, :], self._neuron, 4, activation='relu'))
                splits[-1] = tflearn.flatten(splits[-1])

            merge_net = tflearn.merge(splits, 'concat')
            dense_nets = [merge_net]
            for i in range(self._layer):
                dense_nets.append(tflearn.fully_connected(dense_nets[-1], self._neuron, activation='relu'))
            out = tflearn.fully_connected(dense_nets[-1], self.a_dim, activation='softmax')
...
class CriticNetwork(object):
    ...
    """
    def __init__(self, sess, state_dim, learning_rate, neuron, layer, phy):
        self.sess = sess
        self.s_dim = state_dim
        self.lr_rate = learning_rate
    ...
    def create_critic_network(self):
        with tf.variable_scope('critic'):
            ...
            for i in range(self._layer):
                dense_nets.append(tflearn.fully_connected(dense_nets[-1], self._neuron, activation='relu'))
            out = tflearn.fully_connected(dense_nets[-1], 1, activation='linear')
